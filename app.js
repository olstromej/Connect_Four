...
for (let i = 0; i < squares.length; i++) {
    squares[i].onclick = () => {
        // this will access the array of squares with a class named bottom and adds 7 because that's how many classes have the bottom class
        if(squares[i+7].classList.contains("bottom") &&!squares[i].classList.contains("bottom")) {
            if(currentPlayer === 1) {
                squares[i].classList.add("bottom")
                squares[i].classList.add('player-one')
                currentPlayer = 2
                // changes the player to two from one
